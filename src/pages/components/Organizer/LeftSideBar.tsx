...
function LeftSideBar() {
  ...
  return (
    <> 
      {/* Desktop view */}
      <motion.div 
        className='hidden overflow-y-auto overflow-x-hidden h-full md:flex flex-col items-center gap-2 bg-white shadow-2xl text-xl text-gray-800 relative'
        animate={{
          ...
          {sideBarLinks.map((link, index) => (
            ...
            >
              <NavLink 
                to={link.path}
                className={({isActive}) => `
                  px-4 py-3 items-center justify-center md:justify-start  rounded space-x-2 flex font-semibold border-l-6 
                  hover:bg-purple-700 hover:text-gray-50 hover:border-transparent transition-time
                  ${isActive ? 
                    (isSidebarCollapsed ? 
                      'rounded-3xl border-r-6 w-fit border-purple-700 p-2' 
                      : 'bg-gray-950/10 text-purple-700 rounded border-purple-700') 
                    : 'border-transparent'
                  }`
                }
              >
      ...
      <div className="md:hidden flex  fixed w-full h-fit bg-gray-100 px-5 py-3 shadow shadow-amber-50 bg-am">
        ...
        <AnimatePresence>
          {isOpen &&(
            <>
              {/* closeMenu */}
              <motion.div className=" fixed inset-0 bg-black/10 z-[1000]"
              onClick={closeMenu}/>
              ...
              className="fixed top-0 left-0 w-64 h-full bg-gray-900 shadow-lg z-[1010] p-6 space-y-10 md:hidden"
